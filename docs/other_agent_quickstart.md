...
> 用 `podcast_analysis_doc` 处理这个播客任务。先判断你手头的主材料是什么；如果有音频或转录稿，正文必须优先基于这些主材料，而不是页面信息。信息足够时先生成 Markdown 导读，再导出 `.docx`。如果信息不足，就明确告诉我缺什么，不要硬写完整章节导读。
...
> 如果当前只有简介或 show notes，没有 transcript，而我也没给你音频转录稿，请先停在“轻量版导读”或向我要 transcript，不要编造时间戳和完整章节内容。
